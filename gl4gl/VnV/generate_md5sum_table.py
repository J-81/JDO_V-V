...
def get_md5sum_on_file(file: Path, cache: dict) -> str:
...
def write_md5sum_excel(df: pd.DataFrame, output_file: Path) -> Path:
    """
    Params:
    ...
    df = df.loc[~df['md5sum Excel Sheet'].isnull()] # removes all files without a sheet target
    ...
    df['filepath'] = df['relativePath'].str.replace("processing_scripts_NOpaths","processing_scripts")

    # based on 'md5sum Excel Sheet' key
    for sheet in df['md5sum Excel Sheet'].unique():
        print(f"Working on sheet name: {sheet}")
        df_file_sheet = df.loc[df["md5sum Excel Sheet"] == sheet][['filepath','fileName','md5sum']]
        ...
        for idx, col in enumerate(df_file_sheet.columns):
            max_len = max((df_file_sheet[col].astype(str).map(len).max()), len(str(col))) + COLUMN_LENGTH_BUFFER # max of either longest column value or column header plus a buffer character
            print(f"AutoSpacing: {max_len} sheet:{sheet} col:{col}")
            worksheet.column_dimensions[get_column_letter(idx+1)].width = max_len
        writer.save()

def generate_dataframe(config_fs_f, runsheet, template, root_dir, cache):
    ...
    df['md5sum'] = df['pathObj'].apply(get_md5sum_on_file, args=[cache])
...
def main(root_dir: Path, runsheet: Path, template: str, config_fs_f: Path, outputDir: Path = None):
    """
    Params:
        root_dir: top level directory for a dataset
        runsheet: metadata csv file that MUST contain a column 'sample_name' denoting the sample IDs
    ...
    if md5sum_table_file.exists():
        ...
        df = generate_dataframe(config_fs_f = config_fs_f, runsheet = runsheet, template = template, root_dir = root_dir, cache = cache)
    ...
    write_md5sum_excel(df, output_file = excel_output_file)
    

if __name__ == '__main__':
    main(root_dir = Path(sys.argv[1]), runsheet = Path(sys.argv[2]), template = "Bulk_RNASeq:PairedEnd", config_fs_f=sys.argv[3])
